...
class ImageDatabase:
    ...
    async def get_all_images(self):
        ...
        for r in data:
            print(f"UserID: {r['userid']}\nImage Link: {r['imagelink']}\nTime Created: {r['timecreated']}\nPrompt: {r['prompt']}\nCaption {r['caption']}\n")

        await conn.close()

    async def get_all_images_from_user(self, username:str):
        """
        Return a list of all images generated by a particular user
        """
        conn = await asyncpg.connect(user='postgres', password='chandlerb', 
                                    database='postgres', host='127.0.0.1')
        
        data = await conn.fetch('''SELECT * FROM GeneratedImages where UserID something something something''')

        for r in data:
            print(f"{r['userid']} created image {r['filename']} at {r['timecreated']} with prompt {r['prompt']} and caption {r['caption']}")
